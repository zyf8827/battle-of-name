...
export function ResultPage() {
  ...
  const finalSnapshot = result.snapshots[result.snapshots.length - 1] ?? result.snapshots[0];
    ...
    <section className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-8 p-6 md:p-8">
      <header className="relative overflow-hidden rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-900/40 to-slate-900/80 p-8 shadow-2xl backdrop-blur-sm">
        ...
        <div className="relative z-10 text-center">
          <h2 className="mb-2 text-3xl font-black tracking-widest text-emerald-400 drop-shadow-sm">🏆 战斗结算</h2>
          <div className="mt-4 flex flex-col items-center justify-center gap-2">
            <span className="text-sm font-semibold uppercase tracking-wide text-emerald-600/80">Winner</span>
            <span className="text-5xl font-black text-white drop-shadow-lg">{winner?.name ?? winnerId}</span>
          </div>
          <div className="mt-6 flex items-center justify-center gap-4 text-xs font-medium text-slate-400">
            <span className="rounded-full bg-slate-800/50 px-3 py-1 ring-1 ring-slate-700">总回合: {result.summary.totalRounds}</span>
            <span className="rounded-full bg-slate-800/50 px-3 py-1 ring-1 ring-slate-700">种子: {result.seed}</span>
          </div>
        </div>
      </header>
