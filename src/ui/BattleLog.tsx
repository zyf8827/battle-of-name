...
function stripDuplicatedActorPrefix(text: string, actorName?: string): string {
  ...
  const stripped = trimmed.slice(actorName.length).replace(/^[\s,，:：;；、\-]+/, '');
...
export function BattleLog({ logs, title = '战斗日志' }: BattleLogProps) {
  ...
  return (
    <section className="flex h-[72vh] flex-col overflow-hidden rounded-xl border border-slate-700 bg-slate-900/90 shadow-2xl backdrop-blur-sm">
      ...
      <div ref={containerRef} className="flex-1 overflow-y-auto scroll-smooth px-4 py-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-700">
        <div className="space-y-6">
          {grouped.map(([round, roundLogs]) => (
            <div key={`round-${round}`} className="relative pl-4 before:absolute before:bottom-0 before:left-0 before:top-0 before:w-0.5 before:bg-slate-800">
              <div className="mb-2 flex items-center gap-2 text-xs font-bold text-sky-400">
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-sky-500/10 text-[10px] ring-1 ring-sky-500/30">R</span>
                <span>第 {round} 回合</span>
              </div>
              <ul className="space-y-2.5">
                {roundLogs.map((log) => (
                  <li key={`${log.seq}-${log.eventId ?? 'none'}`} className="group relative flex items-start gap-3 rounded-lg p-1.5 transition-colors hover:bg-slate-800/30">
                    ...
                    <span className="min-w-0 flex-1 text-sm leading-relaxed text-slate-300">
                      ...
                      <span className={log.actorName ? "text-slate-300" : "text-slate-400 italic"}>
