...
export function UnitPanel({ unit, winner }: UnitPanelProps) {
  ...
  return (
    <article
      className={`overflow-hidden rounded-lg border p-3 transition-all ${winner ? 'border-amber-500/30 bg-amber-950/20' : 'border-slate-800 bg-slate-950/40'}`}
    >
      ...
      <div className="mb-1 flex items-center justify-between text-[10px] font-medium text-slate-400">
        <span>
          HP {unit.state.hp} / {unit.state.maxHp}
        </span>
        {unit.state.shield > 0 && (
          <span className="text-sky-400">🛡️ {unit.state.shield}</span>
        )}
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1">
        {unit.modifiers.length === 0 ? (
          <span className="text-[10px] text-slate-600">无状态</span>
        ) : null}
        {unit.modifiers.map((modifier) => (
          <span
            key={`${unit.id}-${modifier.id}-${modifier.appliedOrder}`}
            className="inline-flex items-center gap-1 rounded border border-indigo-500/10 bg-indigo-500/5 px-1.5 py-0.5 text-[10px] text-indigo-300"
          >
            {modifier.name}
