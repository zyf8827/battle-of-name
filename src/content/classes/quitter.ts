...
const tacticalSleep: Modifier = {
  ...
  hooks: {
    onIncoming: (event, { engine, owner }) => {
      // 检查是否致死
      if (['ATTACK', 'DIRECT_DAMAGE'].includes(event.type) && (event.payload.value ?? 0) >= owner.state.hp) {
        ...
        // 触发锁血：伤害置为 0
        // 同时触发回血
        engine.event.emitDirectHeal(owner, owner, Math.floor(owner.state.maxHp * 0.2), ['heal', 'talent']);
