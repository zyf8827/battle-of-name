...
本项目在开发过程中综合使用了多个 AI Agent 工具，以验证不同 AI 服务的协作能力和互补性：

| AI Agent | 主要用途 |
|-----------|----------|
| **Claude Code** | 核心代码生成、架构设计、平衡性调优、Git 提交管理 |
| **GLM (智谱清言)** | 代码片段生成、算法实现、数据分析脚本编写 |
| **Gemini CLI** | 代码审查、注释生成、文档优化 |
| **GitHub Copilot** | 代码补全、语法提示、重构建议 |

**多 Agent 协作模式**：
- 主要依赖 **Claude Code** 作为主 Agent，负责项目整体架构和关键模块实现
- 使用 **GLM** 处理特定算法（如平衡性仿真、权重计算）
- 通过 **Gemini CLI** 进行代码审查和质量把控
- **GitHub Copilot** 作为实时辅助工具，提升编码效率
